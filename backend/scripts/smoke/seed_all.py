...
def get_database_url() -> str:
    ...
    if not database_url:
        raise RuntimeError(
            ...
            "Ejecuta este script dentro del contenedor backend:\n"
            "docker compose -f docker-compose.dev.yml exec backend "
            'sh -lc "PYTHONPATH=/app python /app/scripts/smoke/seed_all.py"'
        )
    return database_url
    ...
    *,
    ...
    if user:
        ...
        if updated:
            ...
            log_ok(f"{role} actualizado: {email}")
        else:
...
def main() -> None:
    log_info("Iniciando seed completo...")
    ...
    try:
        ...
        platform_admin = get_or_create_user(
            session,
            email="platform@jcj.local",
            password="Admin123!",
            role="platform_admin",
            school_id=None,
        )

        school_admin = get_or_create_user(
            session,
        ...
        print("")
        log_ok("Seed completo finalizado")
        print("")
        print("Credenciales:")
        print("  platform_admin : platform@jcj.local / Admin123!")
        print("  school_admin   : admin.smoke@jcj.local / Admin123!")
        print("  teacher        : teacher.smoke@jcj.local / Teacher123!")
        print("  parent         : parent.smoke@jcj.local / Parent123!")
        print("  student        : student.smoke@jcj.local / Student123!")
        print("")
        print("Resumen:")
        print(f"  School         : {school.name}")
        print(f"  Student model  : {student.full_name}")
        print(f"  Guardian       : {guardian.full_name}")
        print(f"  Class          : {classroom.name}")
        print(f"  Student user   : {student_user.email}")
        print("")
        print("Nota:")
        print("  El usuario con role=student es una cuenta de login.")
        print("  El registro en tabla students es la entidad académica del alumno.")
        print(
            "  Si luego quieres vincular ambas cosas explícitamente, habría que modelarlo."
        )
