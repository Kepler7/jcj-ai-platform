...
class AIFallbackEvent(Base):
    ...
    report_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("student_reports.id"), nullable=False
    )
    ai_report_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("ai_reports.id"), nullable=True
    )

    topic_nucleo: Mapped[Optional[List[str]]] = mapped_column(JSONB, nullable=True)
